...
namespace game {
    ...
    static bool playerDbLoad(const char* name, PlayerDb* data) {
        void* stmt =
            utils::sqliteStatement("SELECT data, length(data) FROM player WHERE name = ?;");
        bool result = false;
        utils::sqliteBindText(stmt, 1, name);
        ...
        for (const auto& entry : scene->extras) {
            ...
            if (p) {
                ...
                if (engineAutoRun()) {
                    player->autoRun = true;
                    utils::info(
                        "player: ENGINE_AUTO_RUN set, auto-running from spawn "
                ...
                engine::Transform* transform =
                    getComponent(scene, engine::Transform, playerEntity->id);
                ...
                engine::Camera* camera =
                    getComponent(engine::ecs.defaultScene, engine::Camera, camEntity->id);
    ...
    static bool footOnGround(engine::Entity* footBone) {
        ...
        engine::WorldTransform* w =
            getComponent(footBone->scene, engine::WorldTransform, footBone->id);
        if (!w) return false;
    ...
    static void playerFootsteps(engine::Entity* playerEntity, bool isMoving) {
        ...
        engine::Entity* leftFoot =
            engine::animationGetBoneEntity(playerEntity, "mixamorig:LeftFoot");
        engine::Entity* rightFoot =
            engine::animationGetBoneEntity(playerEntity, "mixamorig:RightFoot");
    ...
    static void kickLightAttach(engine::Entity* playerEntity) {
        ...
        if (kickLightEntity) {
            engine::F_sceneRemoveComponent(kickLightEntity->scene,
                                           kickLightEntity->id,
                                           &engine::Light_id);
    ...
    static void kickLightUpdate(void) {
        ...
        engine::Light* light =
            getComponent(kickLightEntity->scene, engine::Light, kickLightEntity->id);
        if (light) {
    ...
    static void playerLightCreate(engine::Entity* playerEntity) {
        ...
        engine::Transform* transform =
            createComponent(playerEntity->scene, engine::Transform, playerLightEntity->id);
        ...
        engine::Light* light =
            createComponent(playerEntity->scene, engine::Light, playerLightEntity->id);
        light->lightType    = engine::LIGHT_POINT;
        light->color[0]     = 1.0f;
        light->color[1]     = 0.86f;
        light->color[2]     = 0.65f;
    ...
    static void playerLightUpdate(void) {
        ...
        engine::Transform* playerTransform =
            getComponent(playerScene, engine::Transform, playerEntityId);
    ...
    static void fireballUpdate(void) {
        ...
        for (int read = 0; read < fireballCount; read++) {
            ...
            // Move the fireball
            engine::Transform* t =
                getComponent(fb->entity->scene, engine::Transform, fb->entity->id);
            if (t) {
        ...
        if (player->isCasting) {
            ...
            if (nearEnd && castingRepeatsLeft > 0) {
                ...
                if (castingRepeatsLeft > 0) {
                    ...
                    if (player->isMoving)
                        engine::animationPlayBlended(playerEntity,
                                                     ANIM_RUN,
                                                     ANIM_SPEED_RUN,
                                                     true,
                                                     0.2f);
            ...
            if (kickHitboxEntity) {
                ...
                if (footBone) {
                    engine::WorldTransform* footWorld =
                        getComponent(playerEntity->scene, engine::WorldTransform, footBone->id);
                    engine::Transform* hbTransform = getComponent(kickHitboxEntity->scene,
                                                                  engine::Transform,
                                                                  kickHitboxEntity->id);
    ...
    static bool playerIsLocomotionClip(const char* clipName) {
        for (size_t i = 0; i < sizeof(playerLocomotionClips) / sizeof(playerLocomotionClips[0]);
             i++) {
    ...
    static engine::AnimationInstance* playerFindAnimationInstance(engine::Animator* animator,
                                                                  const char* clipName) {
        if (!animator) return nullptr;
        for (i32 i = static_cast<i32>(static_cast<i32>(animator->activeInstances.size())) - 1;
             i >= 0;
             i--) {
    ...
    static void playerPlayLocomotionBlend(engine::Entity* entity,
        ...
        if (!animator) {
            animator          = createComponentTDtor(entity->scene,
                                                     engine::Animator,
                                                     entity->id,
        ...
        playerSetAnimationWeight(animator,
                                 ANIM_STRAFE_LEFT,
                                 speed,
                                 leftWeight,
                                 blendDuration,
    ...
    static void playerMovementTopDownOnly(void) {
        ...
        engine::Camera* camera =
            getComponent(engine::ecs.defaultScene, engine::Camera, cameraEntity->id);
        engine::Transform* cameraTransform =
            getComponent(engine::ecs.defaultScene, engine::Transform, cameraEntity->id);
        ...
        if (player->waitingForGround) {
            ...
            } else if (engine::heightmapTerrainHasBodyAt(hm,
                                                         transform->pos[0],
                                                         transform->pos[2])) {
        ...
        if (!player->isCasting && engineTpose()) {
            engine::animationPlayBlended(playerEntity, ANIM_TPOSE, ANIM_SPEED_TPOSE, true, 0.0f);
    ...
    static void playerFollowFlyingCamera(void) {
        ...
        engine::Transform* camT =
            getComponent(engine::ecs.defaultScene, engine::Transform, camEntity->id);
