...
# ASAN=1 builds with AddressSanitizer (run.sh exports ASAN_OPTIONS/LSAN_OPTIONS).
# Flag changes require a reconfigure (CMake caches it), so a stamp file tracks
# which flag set the current build tree was configured with.
ASAN_FLAG="${ASAN:+ON}"   # ASAN=1 → ON
[ -n "$ASAN_FLAG" ] || ASAN_FLAG=OFF
STAMP_FILE="$BUILD_DIR/.asan_stamp"
if [ ! -d "$BUILD_DIR" ] || [ "$(cat "$STAMP_FILE" 2>/dev/null)" != "$ASAN_FLAG" ]; then
    cmake -GNinja -DCMAKE_BUILD_TYPE=Debug -DENGINE_ASAN=$ASAN_FLAG -S "$ROOT" -B "$BUILD_DIR"
    echo "$ASAN_FLAG" > "$STAMP_FILE"
fi
