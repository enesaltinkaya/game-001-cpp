...
namespace engine {
    ...
    static void buildIblFileList(void) {
        ...
        iblFileListReady = 1;
        utils::info("vulkanIbl: found %d IBL files in %s",
                    static_cast<i32>(iblFiles.size()),
                    STUDIOLIGHTS_DIR);
    }
    ...
    static void loadEnvironmentFromPath(const char* path) {
        ...
        utils::String probe = {.data      = const_cast<char*>(path),
                               .allocated = 0,
                               .size      = static_cast<u32>(strlen(path)),
                               .onHeap    = false};
    ...
    void vulkanIblCyclePrev(void) {
        if (static_cast<i32>(iblFiles.size()) < 2) return;
        iblCurrentIndex = (iblCurrentIndex + (int)static_cast<i32>(iblFiles.size()) - 1) %
                          (int)static_cast<i32>(iblFiles.size());
        loadEnvironmentFromPath(iblFiles[iblCurrentIndex].data);
    }
    ...
    static void createDitherNoiseTexture(void) {
        ...
        snprintf(cachePath,
                 sizeof(cachePath),
                 "%scache%sibl.bin",
                 utils::platform.dataDirectory,
                 utils::platform.seperator);
        ...
        if (utils::fileExists(cachePath)) {
            ...
            if (fileSize == pixelCount) {
                ...
                utils::warn("vulkanIbl: dither noise cache size mismatch (%u != %u), regenerating",
                            fileSize,
                            pixelCount);
            }
            free(fileData);
    ...
    static void extractSHAndSun(const float* pixels, int width, int height, float sunThreshold) {
        ...
        glm_vec4_copy(vec4{static_cast<float>(shL0[0]),
                           static_cast<float>(shL0[1]),
                           static_cast<float>(shL0[2]),
                           0},
                      ibl.shL0_M0);
        glm_vec4_copy(vec4{static_cast<float>(shL1n[0]),
                           static_cast<float>(shL1n[1]),
                           static_cast<float>(shL1n[2]),
                           0},
                      ibl.shL1_Mn1);
        glm_vec4_copy(vec4{static_cast<float>(shL10[0]),
                           static_cast<float>(shL10[1]),
                           static_cast<float>(shL10[2]),
                           0},
                      ibl.shL1_M0);
        glm_vec4_copy(vec4{static_cast<float>(shL1p[0]),
                           static_cast<float>(shL1p[1]),
                           static_cast<float>(shL1p[2]),
                           0},
                      ibl.shL1_Mp1);
        ibl.hasSH = true;
