    ...
    const void* data, u64 dataSize, int* outWidth, int* outHeight)
{
    ...
    *outHeight = 0;
    ...
    u64 pixelCount = (u64)width * height;
    ...
    int32_t pixelStride = 4 * (int32_t)sizeof(float); // 16
    ...
    for (int32_t ci = 0; ci < chunkCount; ci++) {
        ...
        uint8_t* base  = (uint8_t*)pixels + (u64)chunkY0 * lineStride;
        ...
        for (int16_t ch = 0; ch < decoder.channel_count; ch++) {
            ...
            else if (strcmp(name, "G") == 0) outOff = 1 * (int)sizeof(float);
            else if (strcmp(name, "B") == 0) outOff = 2 * (int)sizeof(float);
            else if (strcmp(name, "A") == 0) outOff = 3 * (int)sizeof(float);

            if (outOff >= 0) {
                info->decode_to_ptr       = base + outOff;
                info->user_pixel_stride   = pixelStride;
                info->user_line_stride    = lineStride;
                info->user_bytes_per_element = (int16_t)sizeof(float);
                info->user_data_type      = (uint16_t)EXR_PIXEL_FLOAT;
            } else {
                // Skip unknown channels
                info->decode_to_ptr = NULL;
            }
        }
