...
namespace engine {
    static const char* vulkanToStringMessageSeverity(VkDebugUtilsMessageSeverityFlagBitsEXT s);
    ...
    void vulkanScreenshotRecord(VulkanImage* swapImg, VulkanBuffer* outReadback) {
        ...
        *outReadback       = vulkanCreateReadbackBuffer("screenshot", bufSize, 0);
        ...
        vulkanCopy(.cmd = cmd, .source.img = swapImg, .target.buf = outReadback);
    ...
    void vulkanScreenshotWriteJpg(const VulkanBuffer* readback,
                                  const VulkanImage* swapImg,
                                  const char* path) {
    ...
    void vulkanSaveImage(VulkanImage* img, const char* path) {
        ...
        vulkanCopy(.cmd = cmd, .source.img = img, .target.buf = &readback);
        vulkanBarrier(cmd, DEVICE_WRITE_TO_HOST_READ);
        ...
        if (isFloatFormat(img->format)) {
            ...
            utils::info(
                "vulkanSaveImage: float range ch0=[%.4f,%.4f] ch1=[%.4f,%.4f] ch2=[%.4f,%.4f] "
                "ch3=[%.4f,%.4f]",
                minV[0],
                maxV[0],
                minV[1],
        ...
        if (ok) {
            utils::info("vulkanSaveImage: saved %s (%ux%u, fmt=%d, ch=%d)",
                        path,
                        w,
                        h,
                        img->format,
    ...
    void vulkanSaveImageRaw(VulkanImage* img, const char* path) {
        if (!img || !img->img) {
            utils::warn("vulkanSaveImageRaw: null image");
            return;
        }
        ...
        VulkanBuffer readback = vulkanCreateReadbackBuffer("saveImageRaw", bufSize, 0);
        ...
        if (f) {
            fwrite(pixels, 1, (size_t)bufSize, f);
            fclose(f);
            utils::info("vulkanSaveImageRaw: saved %s (%ux%u, %llu bytes)", path, w, h,
                        (unsigned long long)bufSize);
        } else {
            utils::warn("vulkanSaveImageRaw: failed to open %s", path);
        }
...
#include <dlfcn.h>
    ...
    void vulkanRenderDocCaptureNow(void) {
