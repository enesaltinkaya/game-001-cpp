...
namespace engine {
    ...
    static void cacaoUpdate(VulkanCommand* cmd,
                            VulkanImage* depth,
                            VulkanImage* normals,
                            Camera* camera);
    ...
    static char cacaoEnsureContext(u32 width, u32 height) {
        if (!cacaoBackendReady) {
            ...
            if (!cacaoScratchBuffer) {
                utils::error(
                    "vulkanAOPass: failed to allocate %zu bytes of CACAO backend scratch memory",
            ...
            FfxErrorCode backendResult = ffxGetInterfaceVK(&cacaoBackendInterface,
                                                           device,
                                                           cacaoScratchBuffer,
                                                           cacaoScratchBufferSize,
                                                           1);
        ...
        cacaoOutput =
            vulkanCreateImage(.name   = "CacaoOutput",
        ...
        desc.width                      = width;
        desc.height                     = height;
        desc.useDownsampledSsao =
            true; /* cheaper; bilateral 5x5 upscale reconstructs full-res AO */
    ...
    static void cacaoUpdate(VulkanCommand* cmd,
        ...
        static const float detailOverride = (detailEnv && *detailEnv) ? (float)atof(detailEnv)
                                                                      : -1.0f;
        ...
        if (!(angleOffEnv && *angleOffEnv && atoi(angleOffEnv))) {
            ...
            settings.temporalSupersamplingRadiusOffset =
                1.0f + (((float)phase - 1.0f) / 3.0f) * 0.1f;
        }
    ...
    static char temporalEnsureAccumulators(u32 width, u32 height) {
        ...
        temporalA =
            vulkanCreateImage(.name   = "AoTemporalA",
                              .format = VK_FORMAT_R16G16B16A16_SFLOAT,
                              .usage  = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT |
                              ...
                              .height = (int)height);
        temporalB =
            vulkanCreateImage(.name   = "AoTemporalB",
                              .format = VK_FORMAT_R16G16B16A16_SFLOAT,
                              .usage  = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT |
                                        VK_IMAGE_USAGE_TRANSFER_DST_BIT |
                                        VK_IMAGE_USAGE_TRANSFER_SRC_BIT,
