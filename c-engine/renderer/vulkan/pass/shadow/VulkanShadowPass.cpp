...
namespace engine {
    ...
    static VulkanImage
        shadowMapLayerImages[SHADOW_CASCADE_COUNT]; /* sampled pool proxy per layer */
    ...
    static void buildCascadeMatrix(const Camera* cam,
        ...
        for (int i = 0; i < 8; i++) {
            vec4 wc;
            glm_mat4_mulv((vec4*)cam->cameraUbo.invViewProjectionNoJitter,
                          (float*)&ndcCorners[i],
                          wc);
    ...
    static void ensureShadowMap(int size) {
        ...
        shadowMapImage =
            vulkanCreateImage(.name   = "shadow_csm",
    ...
    static void recreatePipelines(void) {
        ...
        shadowPipeDoubleSided =
            vulkanCreatePipe(.name            = "shadow_csm_ds",
                             .vs              = "shaders/pass/shadow/spv/shadow_csm.vert.spv",
                             .fs              = "shaders/pass/shadow/spv/shadow_csm.frag.spv",
                             .depthFormat     = VK_FORMAT_D32_SFLOAT,
                             .depthClamp      = 1,
    ...
    void vulkanShadowPassSetQuality(ShadowQuality quality) {
        ...
        utils::info("vulkanShadowPass: shadow quality %s -> %s",
                    shadowQualityName(previous),
                    shadowQualityName(quality));
    }
