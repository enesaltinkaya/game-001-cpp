...
void main() {
    ...
    float cascadeShadow = shadowFull.a * contactShadow;
    #if SHADOW_DEBUG > 0
    ...
    // Ambient / IBL
    vec3 ambientDiffuse = vec3(0.8) * baseColor.rgb;
    vec3 ambientSpecular = vec3(0.0);
    if (sceneBuffer.ibl.enabled != 0u) {
        vec3 F_ibl  = fresnelSchlickRoughness(NdotV, F0, roughness);
        vec3 kD_ibl = (1.0 - F_ibl) * (1.0 - metallic);

        // Use the full perturbed normal for IBL lookups (matches Blender EEVEE).
        vec3 R = reflect(-V, N);
        ...
        // Blend prefilter cubemap toward irradiance at high roughness
        // (EEVEE-style: 0.7→0.9, squared ramp).
        const float BLEND_START = 0.7;
        const float BLEND_END   = 0.9;
        float blendScale = 1.0 / (BLEND_END - BLEND_START);
        float specBlend  = clamp((roughness - BLEND_START) * blendScale, 0.0, 1.0);
        specBlend *= specBlend;

        if (sceneBuffer.ibl.prefilterMapIndex != 0u &&
            sceneBuffer.ibl.brdfLutIndex != 0u) {
            // Diffuse: use full perturbed normal for irradiance lookup.
            vec3 irradiance;
            ...
            vec3 specIrradiance;
            if (sceneBuffer.ibl.irradianceMapIndex != 0u) {
                specIrradiance = sampleIrradiance(N);
            } else if (sceneBuffer.ibl.hasSH != 0u) {
                specIrradiance = evaluateSHIrradiance(N);
            } else {
                specIrradiance = sampleEnvironment(N, sceneBuffer.ibl.environmentMapMaxLod);
            }
            vec3 specColor = mix(prefilteredColor, specIrradiance / PI, specBlend);
            ...
            vec2 brdf = texture(sampler2D(textures[nonuniformEXT(sceneBuffer.ibl.brdfLutIndex)],
                            ...
                            .rg;
            // Standard split-sum approximation (matches Blender EEVEE).
            vec3 specFactor = F0 * brdf.x + brdf.y;
            vec3 kD_ibl_ec = (1.0 - specFactor) * (1.0 - metallic);
            vec3 diffuseIBL = kD_ibl_ec * irradiance * baseColor.rgb / PI;
            vec3 specularIBL = specColor * specFactor;
            ambientDiffuse   = diffuseIBL * iblIntensity;
            ambientSpecular  = specularIBL * iblIntensity * iblSpecIntensity;
        } else {
            float maxEnvLod  = sceneBuffer.ibl.environmentMapMaxLod;
            vec3 irradiance;
            if (sceneBuffer.ibl.hasSH != 0u) {
                irradiance = evaluateSHIrradiance(N);
            } else {
                irradiance = sampleEnvironment(N, maxEnvLod);
            }
            vec3 diffuseIBL  = kD_ibl * irradiance * baseColor.rgb / PI;
            float envSpecLod = sqrt(roughness) * maxEnvLod;
            vec3 envSpec     = sampleEnvironment(R, envSpecLod);
            vec3 specularIBL = envSpec * F0;
            ambientDiffuse   = diffuseIBL * iblIntensity;
            ambientSpecular  = specularIBL * iblIntensity * iblSpecIntensity;
        }
    }
    ...
    // contact darkening remains visible even in strongly sun-lit scenes.
    float shadowAmbientFade = mix(0.3, 1.0, mix(1.0, cascadeShadow, NdotL));
    vec3 color = (ambientDiffuse * shadowAmbientFade + ambientSpecular) * ao + Lo * directAo;
