...
void main() {
    ...
    if ((material.featureMask & (1u << MAT_HAS_TEXTURE_OCCLUSION)) != 0u) {
        vec2 uv = inUV * material.occlusionOffsetScale.zw + material.occlusionOffsetScale.xy;
        materialAo = sampleMaterialTexture(material.occlusionTexture, material.occlusionTextureSampler, uv).r;
    }
